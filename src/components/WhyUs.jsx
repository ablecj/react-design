import React from "react";

const WhyUs = () => {
  return (
    <div className="w-full h-[897px] absolute flex  top-[4445px]">
      <div className="w-1/2 h-[897px] bg-[#1C1F35] absolute flex justify-center items-center">
        <div className="w-[900px] h-[897px] absolute flex  justify-end items-center ">
          <div className="w-[700px] h-[610px] flex  relative z-10 ">
            <img
              src="https://s3-alpha-sig.figma.com/img/60b6/f254/d57e1d77c4855d6115a9508f06155bce?Expires=1708300800&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=g8sxYxbvHecNid6i~tmQeJYd8yjAfYlo8aEG8hWnxfyTjHMDgixMfERQHFYTfUdWgZs2bvN41pst~caFe4WguIcv7riDgoj1ycXuuXyDtyBXM67mnY2dXpyIv6gIorfTHCN7rpe0dBC~ZeeljLf4r9OpsSE7JnPpo-MClCkYbV3XtlE4mI-qhdRucH7S-Gq8R1uoVvU4wdHjYVuoX2LQyltHLczDEtCX2J58lfLZcCWbEHq0JcQGUamWRhRUNBoUQ4pGTC0xoXCrGIobQqTJiQLWbP1LsNhqbgpKrmsjvl0oqTSKiidSWzePJYcUqkER5Z~zzX-tgQW3H2GqyBo9GQ__"
              alt=""
              className="object-cover"
            />
          </div>
        </div>
        <div className="w-[317px] h-[139px] relative flex top-[235px] z-20 left-14 bg-gradient-to-r from-yellow-500 via-yellow-400 to-yellow-300 justify-center items-center">
          <div className="w-[255px] h-[80px] flex ">
            <svg
              width="65"
              height="80"
              viewBox="0 0 65 80"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M64.5 42C64.5 59.9565 50.1659 74.5 32.5 74.5C14.8341 74.5 0.5 59.9565 0.5 42C0.5 24.0435 14.8341 9.5 32.5 9.5C50.1659 9.5 64.5 24.0435 64.5 42Z"
                fill="#FFE6A5"
                stroke="#1C1F35"
              />
              <path
                d="M48.5 13.5C48.5 20.6627 42.4731 26.5 35 26.5C27.5269 26.5 21.5 20.6627 21.5 13.5C21.5 6.33728 27.5269 0.5 35 0.5C42.4731 0.5 48.5 6.33728 48.5 13.5Z"
                fill="#FFE6A5"
                stroke="#1C1F35"
              />
              <path
                d="M60.5 66.5C60.5 73.6627 54.4731 79.5 47 79.5C39.5269 79.5 33.5 73.6627 33.5 66.5C33.5 59.3373 39.5269 53.5 47 53.5C54.4731 53.5 60.5 59.3373 60.5 66.5Z"
                fill="#FFE6A5"
                stroke="#1C1F35"
              />
              <path
                d="M35.0261 20.9304C39.0122 20.9304 42.2435 17.6991 42.2435 13.713C42.2435 9.72694 39.0122 6.49561 35.0261 6.49561C31.0401 6.49561 27.8087 9.72694 27.8087 13.713C27.8087 17.6991 31.0401 20.9304 35.0261 20.9304Z"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M28.3639 11.4923H41.6883"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M28.3639 15.9337H41.6883"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M35.0261 20.3752C36.5592 20.3752 37.802 17.3924 37.802 13.713C37.802 10.0336 36.5592 7.0508 35.0261 7.0508C33.493 7.0508 32.2502 10.0336 32.2502 13.713C32.2502 17.3924 33.493 20.3752 35.0261 20.3752Z"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M44.0131 47.3867L55.4294 40.4743"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M42.1451 48.3809L36.9805 51.3627"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M9.52037 46.9932L17.2363 54.2267C18.2651 55.1923 19.5778 55.7997 20.979 55.9584C22.3803 56.1172 23.7953 55.8188 25.0136 55.1078L56 37.0752L52.1932 32.4032C51.2029 31.1923 49.8128 30.3759 48.2739 30.1015C46.7349 29.8272 45.1488 30.113 43.8019 30.9073L34.7147 36.2556L22.4348 32.1573L19.0578 33.6122C18.8052 33.7206 18.5838 33.8909 18.4142 34.1075C18.2446 34.3241 18.1322 34.5799 18.0874 34.8514C18.0425 35.1229 18.0667 35.4014 18.1576 35.6611C18.2485 35.9208 18.4032 36.1535 18.6075 36.3375L24.8908 41.9932L19.1601 45.2719L13.4295 42.8129L9.9911 44.2883C9.74095 44.3959 9.52154 44.5642 9.35273 44.7781C9.18392 44.9919 9.07104 45.2446 9.0243 45.5131C8.97757 45.7817 8.99845 46.0576 9.08507 46.3161C9.17169 46.5745 9.3213 46.8072 9.52037 46.9932V46.9932Z"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M22.7484 52.6577L34.1647 45.7453"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M53.513 70.3391V63.6787C53.5125 63.5825 53.4866 63.4881 53.4379 63.4051C53.3892 63.3221 53.3194 63.2533 53.2356 63.2058L47.2813 59.8621C47.201 59.8158 47.11 59.7915 47.0174 59.7915C46.9247 59.7915 46.8337 59.8158 46.7535 59.8621L40.7991 63.2058C40.7153 63.2533 40.6456 63.3221 40.5969 63.4051C40.5482 63.4881 40.5222 63.5825 40.5217 63.6787V70.3391C40.5222 70.4353 40.5482 70.5297 40.5969 70.6127C40.6456 70.6957 40.7153 70.7645 40.7991 70.812L46.7535 74.1557C46.8337 74.2019 46.9247 74.2263 47.0174 74.2263C47.11 74.2263 47.201 74.2019 47.2813 74.1557L53.2356 70.812C53.3194 70.7645 53.3892 70.6957 53.4379 70.6127C53.4866 70.5297 53.5125 70.4353 53.513 70.3391V70.3391Z"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M53.513 63.4002L47.079 67.0089L40.5217 63.4002"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M47.0174 67.0089V74.2263"
                stroke="#1C1F35"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
            <p
              className="text-[#1C1F35] flex justify-center items-center ml-2 
            font-rubik text-lg font-normal leading-7 tracking-normal text-left"
            >
              Moving your products across borders
            </p>
          </div>
        </div>
      </div>

      <div className="w-1/2 h-[897px] bg-[#F4F4F4] relative ml-[760px] flex justify-center items-center ">
        <div className="w-[553px] h-[497px] flex flex-col relative">
          <div className="w-[494px] h-[207px] flex flex-col relative">
            <div className="w-[438px] h-[117px]">
              <div className="w-[102px] h-[23px] flex ">
                <svg
                  width="4"
                  height="23"
                  viewBox="0 0 4 23"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <rect
                    width="4"
                    height="23"
                    fill="url(#paint0_linear_1_382)"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1_382"
                      x1="-0.113402"
                      y1="4.98333"
                      x2="4.94324"
                      y2="5.04578"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#FFB629" />
                      <stop offset="0.507189" stop-color="#FFDA56" />
                      <stop offset="1" stop-color="#FFD7A6" />
                    </linearGradient>
                  </defs>
                </svg>

                <p className="font-rubik text-sm font-normal leading-5 tracking-normal text-left text-[#1C1F35]">
                  Why Choose
                </p>
              </div>
              <h2 className="text-[#1C1F35] font-rubik text-4xl font-semibold leading-[41px] tracking-normal text-left">
                We create opportunity to reach potential
              </h2>
            </div>
            <p className="text-[#666C89] font-krub text-base font-medium leading-6 text-left">
              Leverage agile frameworks to provide a robust synopsis for
              strategy foster collaborative thinking to further the overall
              value proposition.{" "}
            </p>
          </div>
          <div className="w-[553px] h-[229px] relative mt-[72px]  justify-between flex ">
            <div className="w-[223px] h-[229px] flex flex-col gap-[20px]">
              <div className="w-[201px] h-[63px] flex justify-center items-center">
                <svg
                  width="63"
                  height="63"
                  viewBox="0 0 63 63"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle
                    cx="31.5"
                    cy="31.5"
                    r="31.5"
                    fill="url(#paint0_linear_1_390)"
                  />
                  <path
                    d="M43 36.537V25.463C42.9991 25.303 42.9552 25.1461 42.8728 25.0081C42.7903 24.8701 42.6721 24.7558 42.5302 24.6768L32.4469 19.1174C32.311 19.0405 32.1569 19 32 19C31.8431 19 31.689 19.0405 31.5531 19.1174L21.4698 24.6768C21.3279 24.7558 21.2097 24.8701 21.1272 25.0081C21.0448 25.1461 21.0009 25.303 21 25.463V36.537C21.0009 36.697 21.0448 36.8539 21.1272 36.9919C21.2097 37.1299 21.3279 37.2442 21.4698 37.3232L31.5531 42.8826C31.689 42.9595 31.8431 43 32 43C32.1569 43 32.311 42.9595 32.4469 42.8826L42.5302 37.3232C42.6721 37.2442 42.7903 37.1299 42.8728 36.9919C42.9552 36.8539 42.9991 36.697 43 36.537V36.537Z"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M38 34V28.0853L27 22"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M43 25L32.1043 31L21 25"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M32 31V43"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1_390"
                      x1="-1.78608"
                      y1="13.65"
                      x2="77.4684"
                      y2="19.2782"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#FFB629" />
                      <stop offset="0.507189" stop-color="#FFDA56" />
                      <stop offset="1" stop-color="#FFD7A6" />
                    </linearGradient>
                  </defs>
                </svg>
                <p className="font-rubik text-lg font-normal leading-6 tracking-normal text-left w-[126px] h-[24px] ml-1 text-[#1C1F35]">
                  Safe Package
                </p>
              </div>

              <div className="w-[216px] h-[63px] flex justify-center items-center">
                <svg
                  width="63"
                  height="63"
                  viewBox="0 0 63 63"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle
                    cx="31.5"
                    cy="31.5"
                    r="31.5"
                    fill="url(#paint0_linear_1_399)"
                  />
                  <path
                    d="M32 20C38.6274 20 44 25.3726 44 32C44 38.6274 38.6274 44 32 44C25.3726 44 20 38.6274 20 32C20 25.3726 25.3726 20 32 20Z"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M21 36H43"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M21 28H43"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M32 20C34.2091 20 36 25.3726 36 32C36 38.6274 34.2091 44 32 44C29.7909 44 28 38.6274 28 32C28 25.3726 29.7909 20 32 20Z"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1_399"
                      x1="-1.78608"
                      y1="13.65"
                      x2="77.4684"
                      y2="19.2782"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#FFB629" />
                      <stop offset="0.507189" stop-color="#FFDA56" />
                      <stop offset="1" stop-color="#FFD7A6" />
                    </linearGradient>
                  </defs>
                </svg>
                <p className="w-[141px] h-[24px] text-[#1C1F35] ml-1 font-rubik text-base font-normal leading-6 tracking-normal text-left">
                  Global Tracking
                </p>
              </div>

              <div className="w-[223px] h-[63px] flex justify-center items-center">
                <svg
                  width="63"
                  height="63"
                  viewBox="0 0 63 63"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle
                    cx="31.5"
                    cy="31.5"
                    r="31.5"
                    fill="url(#paint0_linear_1_408)"
                  />
                  <path
                    d="M32 42C38.0751 42 43 37.0751 43 31C43 24.9249 38.0751 20 32 20C25.9249 20 21 24.9249 21 31C21 37.0751 25.9249 42 32 42Z"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M32 25V31H39"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1_408"
                      x1="-1.78608"
                      y1="13.65"
                      x2="77.4684"
                      y2="19.2782"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#FFB629" />
                      <stop offset="0.507189" stop-color="#FFDA56" />
                      <stop offset="1" stop-color="#FFD7A6" />
                    </linearGradient>
                  </defs>
                </svg>
                <p className="w-[148px] h-24px] text-[#1C1F35] font-rubik text-base font-normal leading-6 tracking-normal text-left ml-1">
                  In Time Delivery
                </p>
              </div>
            </div>

            <div className="w-[258px] h-[229px] flex flex-col gap-[20px]">
              <div className="w-[220px] h-[63px] flex justify-center items-center">
                <svg
                  width="63"
                  height="63"
                  viewBox="0 0 63 63"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle
                    cx="31.5"
                    cy="31.5"
                    r="31.5"
                    fill="url(#paint0_linear_1_416)"
                  />
                  <path
                    d="M32 23V20"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M42 34.487C40.4792 39.8216 33.4375 41.6835 32.1875 41.9764C32.0645 42.0079 31.9355 42.0079 31.8125 41.9764C30.5625 41.6835 23.5208 39.8216 22 34.487V30.9096C22 30.7334 22.0554 30.5617 22.1582 30.4189C22.2611 30.2762 22.4062 30.1697 22.5729 30.1146L31.7396 27.0393C31.9093 26.9869 32.0907 26.9869 32.2604 27.0393L41.4271 30.1146C41.5938 30.1697 41.7389 30.2762 41.8418 30.4189C41.9446 30.5617 42 30.7334 42 30.9096V34.487Z"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M32 35V27"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M25 29V23.75C25 23.5511 25.0878 23.3603 25.2441 23.2197C25.4004 23.079 25.6123 23 25.8333 23H39.1667C39.3877 23 39.5996 23.079 39.7559 23.2197C39.9122 23.3603 40 23.5511 40 23.75V29"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1_416"
                      x1="-1.78608"
                      y1="13.65"
                      x2="77.4684"
                      y2="19.2782"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#FFB629" />
                      <stop offset="0.507189" stop-color="#FFDA56" />
                      <stop offset="1" stop-color="#FFD7A6" />
                    </linearGradient>
                  </defs>
                </svg>
                <p className="w-[145px] h-[24px] text-[#1C1F35] font-rubik text-base font-normal leading-6 tracking-normal text-left ml-1">
                  Ship Everyware
                </p>
              </div>

              <div className="w-[199px] h-[63px] flex justify-center items-center">
                <svg
                  width="63"
                  height="63"
                  viewBox="0 0 63 63"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle
                    cx="31.5"
                    cy="31.5001"
                    r="31.5"
                    fill="url(#paint0_linear_1_426)"
                  />
                  <path
                    d="M41 29.143H37.8579C37.4412 29.143 37.0416 29.3035 36.747 29.5893C36.4523 29.8751 36.2868 30.2626 36.2868 30.6668V34.4762C36.2868 34.8804 36.4523 35.2679 36.747 35.5537C37.0416 35.8395 37.4412 36 37.8579 36H39.4289C39.8456 36 40.2452 35.8395 40.5398 35.5537C40.8345 35.2679 41 34.8804 41 34.4762V29.143ZM41 29.143C41 27.9359 40.7536 26.7408 40.275 25.6266C39.7964 24.5123 39.095 23.501 38.2114 22.6511C37.3277 21.8011 36.2793 21.1292 35.1265 20.6743C33.9738 20.2193 32.7396 19.9903 31.4951 20.0003C30.2514 19.9915 29.0183 20.2215 27.8667 20.677C26.7151 21.1326 25.6678 21.8046 24.7852 22.6545C23.9026 23.5043 23.2021 24.5152 22.7241 25.6288C22.246 26.7424 22 27.9367 22 29.143V34.4762C22 34.8804 22.1655 35.2679 22.4602 35.5537C22.7548 35.8395 23.1544 36 23.5711 36H25.1421C25.5588 36 25.9584 35.8395 26.253 35.5537C26.5477 35.2679 26.7132 34.8804 26.7132 34.4762V30.6668C26.7132 30.2626 26.5477 29.8751 26.253 29.5893C25.9584 29.3035 25.5588 29.143 25.1421 29.143H22"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M41 35V37.1429C41 37.9006 40.661 38.6273 40.0575 39.1632C39.454 39.699 38.6356 40 37.7821 40H32"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1_426"
                      x1="-1.78608"
                      y1="13.65"
                      x2="77.4684"
                      y2="19.2783"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#FFB629" />
                      <stop offset="0.507189" stop-color="#FFDA56" />
                      <stop offset="1" stop-color="#FFD7A6" />
                    </linearGradient>
                  </defs>
                </svg>
                <p className="w-[124px] h-[24px] text-[#1C1F35] font-rubik text-base font-normal leading-6 tracking-normal text-left ml-1">
                  24/7 Support
                </p>
              </div>

              <div className="w-[258px] h-[63px] flex justify-center items-center">
                <svg
                  width="63"
                  height="63"
                  viewBox="0 0 63 63"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle
                    cx="31.5"
                    cy="31.5"
                    r="31.5"
                    fill="url(#paint0_linear_1_433)"
                  />
                  <path
                    d="M29.5 42C33.6421 42 37 37.0751 37 31C37 24.9249 33.6421 20 29.5 20C25.3579 20 22 24.9249 22 31C22 37.0751 25.3579 42 29.5 42Z"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M30 20H35.5385C39.1038 20 42 24.9271 42 31C42 37.0729 39.1038 42 35.5385 42H30"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M36 24L39 24"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M37 31H42"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M35 39H40"
                    stroke="#1C1F35"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1_433"
                      x1="-1.78608"
                      y1="13.65"
                      x2="77.4684"
                      y2="19.2782"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#FFB629" />
                      <stop offset="0.507189" stop-color="#FFDA56" />
                      <stop offset="1" stop-color="#FFD7A6" />
                    </linearGradient>
                  </defs>
                </svg>
                <p className="w-[183px] h-[24px] text-[#1C1F35] font-rubik text-base font-normal leading-6 tracking-normal text-left ml-1">Transparant Pricing</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default WhyUs;
